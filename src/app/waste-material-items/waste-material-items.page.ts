...
@Component({
  selector: 'app-waste-material-items',
  templateUrl: 'waste-material-items.page.html',
  styleUrls: ['waste-material-items.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class WasteMaterialItemsPage implements OnInit {
  items: WasteMaterialItemOption[] = [];
  filteredItems: WasteMaterialItemOption[] = [];
  searchQuery: string = '';
  showDeleted: boolean = false;
  isDeveloperMode: boolean = false;
  editingItem: WasteMaterialItemOption | null = null;
  newItemName: string = '';
  ...
  async loadItems() {
    this.isLoading = true;
    try {
      this.items = await this.wasteMaterialItemsService.getAllItems(this.showDeleted);
      this.filterItems();
    } catch (error) {
  ...
  async saveNewItem() {
    ...
    await this.wasteMaterialItemsService.addItem(this.newItemName);
    this.showToast('Item added successfully', 'success');
    await this.loadItems();
    this.cancelAdding();
  }

  startEditing(item: WasteMaterialItemOption) {
  ...
  async saveEdit() {
    ...
    if (originalName) {
      await this.wasteMaterialItemsService.updateItem(originalName, this.editingItem.name);
  ...
  async deleteItem(item: WasteMaterialItemOption) {
    let countdown = 10;
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: async () => {
            ...
            await this.wasteMaterialItemsService.deleteItem(item.name);
  ...
  async restoreItem(item: WasteMaterialItemOption) {
    const alert = await this.alertController.create({
      header: 'Restore Item',
      ...
      buttons: [
        ...
        },
        {
          text: 'Restore',
          handler: async () => {
            await this.wasteMaterialItemsService.restoreItem(item.name);
  ...
  async permanentDeleteItem(item: WasteMaterialItemOption) {
    let countdown = 10;
    let countdownInterval: any;
    let deleteButton: any;
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: async () => {
            ...
            await this.wasteMaterialItemsService.permanentDeleteItem(item.name);
