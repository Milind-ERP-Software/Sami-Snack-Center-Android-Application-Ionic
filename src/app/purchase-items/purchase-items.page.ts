...
import { IonHeader, IonToolbar, IonTitle, IonContent, IonIcon, IonButton, IonInput, IonItem, IonList, IonToggle } from '@ionic/angular/standalone';
import { AlertController, ToastController } from '@ionic/angular';
import { addIcons } from 'ionicons';
import { arrowBack, add, create, trash, checkmark, close, documentText, refreshCircleOutline } from 'ionicons/icons';
import { PurchaseItemsService, PurchaseItemOption } from '../services/purchase-items.service';
import { StorageService } from '../services/storage.service';

@Component({
  selector: 'app-purchase-items',
  templateUrl: 'purchase-items.page.html',
  styleUrls: ['purchase-items.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class PurchaseItemsPage implements OnInit {
  items: PurchaseItemOption[] = [];
  filteredItems: PurchaseItemOption[] = [];
  searchQuery: string = '';
  showDeleted: boolean = false;
  isDeveloperMode: boolean = false;
  editingItem: PurchaseItemOption | null = null;
  newItemName: string = '';
  ...
  constructor(
    ...
    addIcons({ arrowBack, add, create, trash, checkmark, close, documentText, refreshCircleOutline });
  }
  ...
  async loadItems() {
    this.isLoading = true;
    try {
      this.items = await this.purchaseItemsService.getAllItems(this.showDeleted);
      this.filterItems();
    } catch (error) {
  ...
  startEditing(item: PurchaseItemOption) {
  ...
  async deleteItem(item: PurchaseItemOption) {
    let countdown = 10;
  ...
  async restoreItem(item: PurchaseItemOption) {
    const alert = await this.alertController.create({
      header: 'Restore Item',
      ...
      buttons: [
        ...
        },
        {
          text: 'Restore',
          handler: async () => {
            await this.purchaseItemsService.restoreItem(item.name);
  ...
  async permanentDeleteItem(item: PurchaseItemOption) {
    let countdown = 10;
    let countdownInterval: any;
    let deleteButton: any;
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: async () => {
            ...
            await this.purchaseItemsService.permanentDeleteItem(item.name);
